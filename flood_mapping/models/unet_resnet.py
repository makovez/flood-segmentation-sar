...
def get_backbone(name, pretrained=True):
    ...
    if name == 'resnet18':
        backbone = models.resnet18(weights=(models.ResNet18_Weights if pretrained else None))
    elif name == 'resnet34':
        backbone = models.resnet34(weights=(models.ResNet34_Weights if pretrained else None))
    elif name == 'resnet50':
        backbone = models.resnet50(weights=(models.ResNet50_Weights if pretrained else None))
    elif name == 'resnet101':
...
class Unet(nn.Module):
    ...
    def forward_backbone(self, x):
        ...
        if x.size(1) <= 2: # Increase to third channel if necessary with all zeros
            x = torch.cat((x, torch.zeros(x.size(0), 1, x.size(2), x.size(3), device=x.device)), dim=1)
...
if __name__ == "__main__":
    ...
    for _ in range(1):
        with torch.set_grad_enabled(True):
            batch = torch.empty(8, 2, 224, 224).normal_()
            targets = torch.empty(8, 2, 224, 224).normal_()
